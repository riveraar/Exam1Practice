...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n - 1):
        points = rg.Point(rect.get_upper_right_corner().x + ((k + 1) * delta),
                          rect.get_upper_right_corner().y - ((k + 1) * delta))
        pointz = rg.Point(rect.get_lower_left_corner().x - ((k + 1) * delta),
                          rect.get_lower_left_corner().y + ((k + 1) * delta))
        recta = rg.Rectangle(points, pointz)
        recta.attach_to(win)
